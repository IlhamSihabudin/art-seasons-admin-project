...
export const ArtistsEditPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await API.get<ResponseApi<Artist>>(`/artists/${Number(params.id)}`, {
    ...
    <section className='space-y-5'>
      ...
      <form className='grid md:grid-cols-2 md:gap-10 gap-5 container' encType='multipart/form-data' onSubmit={handleSubmit}>
        <fieldset className='md:space-y-7 space-y-3'>
          ...
          <Input label='Birth Year' type='number' min={1900} max={new Date().getFullYear()} placeholder='Enter birth year' required defaultValue={data?.birth_year} ref={birth_year} />
          <Textarea label='Short Description' required placeholder='Enter short description that will appear on the main top banner of the artist page' defaultValue={data?.short_desc} ref={short_desc} />
          <fieldset>
            ...
            <Input type='file' accept='.jpg,.jpeg,.png' required onChange={(e: React.FormEvent<HTMLInputElement>) => {
          ...
          <fieldset>
            ...
            <Input accept='.pdf' type='file' required onChange={(e: React.FormEvent<HTMLInputElement>) => {
              const files = (e.target as HTMLInputElement).files
              if (files !== null) {
                setDoc(files[0])
              }
          ...
          <fieldset>
            ...
            <RadioGroup className='flex items-center'>
              <div className='flex items-center space-x-2'>
                <Input type="radio" value='1' id='visible' required name="isVisible" onChange={e => setIsVisible(e.target.value)} defaultChecked={data && data.is_visible.toString() == '1'} />
                <Label htmlFor='visible' className='font-normal'>
                  Visible
                </Label>
              </div>
              <div className='flex items-center space-x-2'>
                <Input type="radio" value='0' id='hidden' name="isVisible" onChange={e => setIsVisible(e.target.value)} defaultChecked={data && data.is_visible.toString() == '0'}  />
        ...
        <Textarea label='Long description' required placeholder='Enter your comprehensive description on the artist' className='h-full' defaultValue={data?.long_desc} ref={long_desc} />
