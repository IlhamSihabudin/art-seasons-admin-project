...
export const GeneralTab = ({ callback, formInput, form }: { callback: (value: boolean) => void, formInput: (value: ExhibitionForm) => void, form?: ExhibitionForm }) => {
    ...
    <section>
      <form className='grid md:grid-cols-2 md:gap-10 gap-5 container' onSubmit={handleNextTab}>
        <fieldset className='md:space-y-7 space-y-3'>
          <Input label='Exhibition Name' required placeholder='Enter exhibition name' ref={fullname} defaultValue={form?.name}/>
          <fieldset className='grid grid-cols-2 gap-5'>
            <Input label='Start Date' type='date' required placeholder='Enter start date' ref={start} defaultValue={form?.start_date} />
            <Input label='End Date' type='date' required placeholder='Enter end date' ref={end} defaultValue={form?.end_date}/>
          </fieldset>
          <Input label='Tags' placeholder='Enter tags' required ref={tag} defaultValue={form?.tags} />
          <Input label='Organizer' placeholder='Enter artist or organisation name' required ref={orga} defaultValue={form?.organizer} />
          <Input label='Location' placeholder='Enter Location' required ref={loc} defaultValue={form?.location} />
          <fieldset>
          ...
          <fieldset>
            ...
            <RadioGroup className='flex items-center'>
              <div className='flex items-center space-x-2'>
                <input type="radio" value='1' id='visible' required name="isVisible" onChange={(e: React.FormEvent<HTMLInputElement>) => setIsVisible((e.target as HTMLInputElement).value)}/>
                <Label htmlFor='visible' className='font-normal'>
                  Visible
                </Label>
              </div>
              <div className='flex items-center space-x-2'>
                <input type="radio" value='0' id='hidden' name="isVisible" onChange={(e: React.FormEvent<HTMLInputElement>) => setIsVisible((e.target as HTMLInputElement).value)} />
        ...
        <Textarea label='Description' required placeholder='Enter your comprehensive description on the artist' wrapperClassName='flex flex-col' className='flex-1' ref={descrip} defaultValue={form?.desc}/>
