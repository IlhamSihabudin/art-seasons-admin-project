...
export const ViewAction = ({ data }: { data: Exhibition }) => {
  const [artists, setArtists] = useState<ExhibitionDetail>();
  const [tab, setTab] = useState<string>("general");

  const handleOnClick = useCallback( async () => {
    try {
      const fetch = await API.get<ResponseApi<ExhibitionDetail>>(`/exhibitions/${data.id}`);
