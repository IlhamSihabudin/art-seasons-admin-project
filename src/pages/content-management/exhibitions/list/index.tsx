...
import { Exhibition, ResponseApiList } from '@/types/API'
import { useGet } from '@/hooks/useGet'

export const ExhibitionsPage = () => {
  const { data, isLoading } = useGet<ResponseApiList<Exhibition>>("exhibition", "/exhibitions?limit=10000")
  ...
  return (
    <section className='space-y-5'>
      <h1 className='font-bold text-3xl'>Exhibitions</h1>
      <div className='flex items-center gap-3'>
        <Button variant='outline' className='gap-2'>
          <ListFilter size={16} />
          Filters
        </Button>
        <Link to='/content-management/exhibitions/create' className={`${buttonVariants()} gap-2`}>
          <Plus size={16} />
          Add New
        </Link>
