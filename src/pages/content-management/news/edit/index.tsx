...
export const NewsEditPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await API.get<ResponseApi<News>>(`/news/${Number(params.id)}`, {
      ...
      <form className='grid md:grid-cols-2 md:gap-10 gap-5 container' encType='multipart/form-data' onSubmit={handleSubmit}>
        <fieldset className='md:space-y-7 space-y-3'>
          ...
          <fieldset>
            ...
            <RadioGroup className='flex items-center'>
              <div className='flex items-center space-x-2'>
                <input type="radio" value='internal' id='internal' name="catagory" onChange={(e: React.FormEvent<HTMLInputElement>) => setCatagory(e.target.value)} defaultChecked={data && data.category === 'internal'} />
                <Label htmlFor='internal' className='font-normal'>
                  Internal
                </Label>
              </div>
              <div className='flex items-center space-x-2'>
                <input type="radio" value='external' id='external' name="catagory" onChange={(e: React.FormEvent<HTMLInputElement>) => setCatagory(e.target.value)} defaultChecked={data && data.category === 'external'} />
          ...
          <fieldset>
            ...
            <RadioGroup className='flex items-center'>
              <div className='flex items-center space-x-2'>
                <input type="radio" value='1' id='visible' name="isVisible" onChange={(e: React.FormEvent<HTMLInputElement>) => setIsVisible(e.target.value)} defaultChecked={data && data.is_visible == '1'} />
                <Label htmlFor='visible' className='font-normal'>
                  Visible
                </Label>
              </div>
              <div className='flex items-center space-x-2'>
                <input type="radio" value='0' id='hidden' name="isVisible" onChange={(e: React.FormEvent<HTMLInputElement>) => setIsVisible(e.target.value)} defaultChecked={data && data.is_visible == '0'} />
        ...
        <Textarea name="article" className='h-full' placeholder="Enter your article content" required label='Article' ref={article} defaultValue={data?.article} />
