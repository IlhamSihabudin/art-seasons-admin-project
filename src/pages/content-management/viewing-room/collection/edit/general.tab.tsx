...
import { CollectionDetail } from '@/types/API'
import { CollectionBodyRequest } from '../create'

export const GeneralTab = ({ callback, formInput, data }: { callback: (value: boolean) => void, formInput: (value: CollectionBodyRequest) => void, data: CollectionDetail }) => {
    ...
    <section>
      <form className='grid md:grid-cols-2 md:gap-10 gap-5 container'>
        <fieldset className='md:space-y-7 space-y-3'>
          ...
          <Input label='Tags' placeholder='Enter tags' required ref={tag} defaultValue={data?.tags} />
          <Input label='Organizer' placeholder='Enter artist or organisation name' required ref={orga} defaultValue={data?.organizer} />
          <Input label='Location' placeholder='Enter Location' required ref={loc} defaultValue={data?.location} />
          <fieldset>
        ...
        <Textarea label='Description' required placeholder='Enter your comprehensive description on the artist' wrapperClassName='flex flex-col' className='flex-1' ref={descrip} defaultValue={data?.desc}/>
