...
import { ArtworkDetail, Collection, CollectionDetail, ResponseApi, ResponseApiList } from '@/types/API'
...
export const ViewingRoomEditPage = () => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const response = await API.get<ResponseApi<CollectionDetail>>(
          `/viewing-room/collection/${Number(params.id)}`,
          {
  ...
  const handleSubmit = async () => {
    if (validate()) {
      ...
      try {
        await API.post<typeof body, ResponseApi<Collection>>(`/viewing-room/collection/${Number(params.id)}`, body, {
