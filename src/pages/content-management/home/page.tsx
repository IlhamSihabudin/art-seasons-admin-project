...
export type ResponseType<T extends string> = 
  T extends "artists" ? Artist :
  T extends "art fairs" ? ArtFair :
  T extends "exhibitions" ? Exhibition :
  T extends "viewing room" ? Artwork :
  T extends "news" ? ResponseSearchNews :
    ...
    <section className='space-y-5'>
      ...
      <form className='grid md:grid-cols-2 md:gap-10 gap-5 container' onSubmit={handleSubmit}>
        ...
        <fieldset>
          ...
          {currentTopBanner?.length > 0 && currentTopBanner.map((banner) => (
            <div key={banner.id} className="flex mb-2 flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
            ...
            {topBanner?.map((banner, index) => (
              <div key={index} className='pb-1'>
                ...
                <div className='flex items-center flex-1'>
                    <Input placeholder='Logo Name' defaultValue={banner.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      const set = {
                        ...banner,
        ...
        <fieldset>
          ...
          {currentButtomBanner?.length > 0 && currentButtomBanner.map((banner) => (
            <div key={banner.id} className="flex mb-2 flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
              <img className="max-h-36 aspect-square object-center object-cover rounded-l-lg" src={banner.img.toString()} alt={banner.type} />
              <div className="flex flex-col justify-between p-4 leading-normal">
                  <h5 className="mb-2 text-md font-bold tracking-tight text-gray-900 dark:text-white">{banner.name}</h5>
          ...
          <Reorder.Group axis='y' onReorder={setBottomBanner} values={bottomBanner} className='space-y-2.5 overflow-hidden mb-4'>
            {bottomBanner?.map((banner, index) => (
              <div key={index} className='pb-1'>
                ...
                <div className='flex items-center flex-1'>
                    <Input placeholder='Baner Name' defaultValue={banner.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      const set = {
                        ...banner,
                        name: (e.target as HTMLInputElement).value
                      }
        ...
        <fieldset>
          <div className='space-y-2'>
            ...
            {featuredItems && featuredItems.map((feature, index) => (
              <div key={index} className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                ...
                <div className="flex flex-col justify-between p-4 leading-normal">
                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{feature.name ? feature.name : feature.headline ? feature.headline : feature.fullname}</h5>
            ...
            <div className='flex items-center gap-5 relative'>
                      ...
                      {featuredType.map((type) => (
                        ...
                        >
                          ...
                          <CheckIcon
                            className={cn(
                              "ml-auto h-4 w-4",
                              type.value === selectedType?.value ? "opacity-100" : "opacity-0"
                            )}
                          />
                        </CommandItem>
                      ))}
              ...
              <Button type='button' onClick={handleSelectType}>Select</Button>
            </div>
          </div>
      ...
      {listNews.length > 0 && (
        <div className='mt-4 space-y-2'>
          ...
          {latestNews && latestNews.map((news, index) => (
            <div key={index} className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
          ...
          {getSelected.length > 0 && (
            <div className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
              ...
              <div className="flex flex-col justify-between p-4 leading-normal">
                ...
                <Textarea label='Preview:' required onChange={(event) => previewLatestNews(
                  {
          ...
          <Input
            ...
            placeholder='Search...'
            onChange={event => tableLatestNews.getColumn('headline')?.setFilterValue(event.target.value)}
          />
