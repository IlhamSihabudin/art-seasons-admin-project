    ...
    <section>
      <form className='grid md:grid-cols-2 md:gap-10 gap-5 container' onSubmit={handleSubmit}>
        <fieldset className='md:space-y-7 space-y-3'>
          ...
          <Input type="tel" name="telphone" placeholder="081234567890" pattern="[0-9]{12}" maxLength="12" required label='Phone Number' ref={phone} defaultValue={data?.data.phone_number} />
          <fieldset>
        ...
        <fieldset className='md:space-y-7 space-y-3'>
          <div className='flex relative'>
            <Input type='email' disabled={!emailEditable} className='absolute w-[90%]' label='Email' required placeholder='Enter email' ref={email} autoComplete="off" defaultValue={data?.data.email} />
